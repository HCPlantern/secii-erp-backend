...
//            warehouseDao.deductQuantity(warehousePO);
    ...
    public List<WarehouseIODetailPO> getWarehouseIODetailByTime(String beginDateStr, String endDateStr) {
        ...
        List<WarehouseIODetailPO> ioDetail = warehouseInputSheetDao.getWarehouseIODetailByTime(beginDate, endDate);
        return ioDetail == null ? new ArrayList<WarehouseIODetailPO>() : ioDetail;
    }
    ...
    public int getWarehouseInputProductQuantityByTime(String beginDateStr, String endDateStr) {
        ...
        try {
            ...
            Integer quantity = warehouseInputSheetDao.getWarehouseInputProductQuantityByTime(beginDate, endDate);
    ...
    public int getWarehouseOutProductQuantityByTime(String beginDateStr, String endDateStr) {
         ...
         * 1.注意日期的格式转换和转换异常
         * 2.考虑开始时间大于结束时间的情况、查询结果为空的情况
         * 3.Dao层和service层接口已提供，需要先补充WarehouseOutputSheetMapper中的sql语句
         */
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
            Date beginDate = sdf.parse(beginDateStr);
            Date endDate = sdf.parse(endDateStr);
            if (beginDate.after(endDate)) return 0;
            Integer quantity = warehouseOutputSheetDao.getWarehouseOutputProductQuantityByTime(beginDate, endDate);
            return quantity == null ? 0 : quantity;
        } catch (ParseException e) {
            throw new RuntimeException(e);
        }
